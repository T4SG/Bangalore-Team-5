<?php
	 ...
	 */
		...
		if ($vars['entity']->type == group){
			...
			$info .= '<p><a href="'. $vars['url'] . 'action/groups/join?group_guid=' . $guid  . '&__elgg_token=' . $token . '&__elgg_ts=' . $ts . '">'.elgg_echo('grouprequest:approve').'</a> | ';
			$info .= '<a href="'. $vars['url'] . 'action/groups/killinvite?group_guid=' . $guid . '&__elgg_token=' . $token . '&__elgg_ts=' . $ts . '">'.elgg_echo('grouprequest:kill').'</a>';;		
		}else{			
			$info .= "<p><b><a href=\"" . $vars['entity']->getUrl() . "\" rel=\"$rel\">" . $vars['entity']->name . "</a></b></p>";
			...
			$info .= '<a href="'. $vars['url'] . 'action/friendrequest/decline?guid=' . $guid . '&__elgg_token=' . $token . '&__elgg_ts=' . $ts . '">'.elgg_echo('friendrequest:deny').'</a></p>';
		}
